...
function adminRenderCategories() {
  ...
  document.getElementById('admin-categories').innerHTML = `
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:18px;margin-bottom:16px">
      <div class="admin-form-group"><label>اسم التصنيف</label><input type="text" id="acName" placeholder="مثال: جوالات"></div>
      <div class="admin-form-group"><label>صورة التصنيف</label>
        <div style="display:flex;gap:10px;align-items:center">
          <input type="text" id="acImage" placeholder="رابط الصورة أو ارفع صورة" style="flex:1">
          <button class="admin-btn admin-btn-secondary admin-btn-sm" onclick="adminUploadCatImage()"><i class="fa-solid fa-upload"></i></button>
          <img id="acPreview" style="width:48px;height:48px;border-radius:10px;object-fit:cover;display:none;border:2px solid var(--border)">
        </div>
      </div>
      <div class="admin-form-group"><label style="display:flex;align-items:center;gap:8px;cursor:pointer"><input type="checkbox" id="acIsBrand" style="width:16px;height:16px"> علامة تجارية (يظهر في فلتر الماركات)</label></div>
      <button class="admin-btn admin-btn-primary" onclick="adminAddCategory()" style="margin-top:6px"><i class="fa-solid fa-plus"></i> إضافة تصنيف</button>
    </div>
    <div id="adminCategoriesList">
      ${cats.length ? cats.map((c, idx) => `
        <div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--card);border:1px solid var(--border);border-radius:8px;margin-bottom:4px">
          ...
          <div style="flex:1"><strong style="font-size:.85rem">${c.name} ${c.isBrand ? '<span style="font-size:.65rem;background:var(--accent);color:#fff;padding:1px 6px;border-radius:4px;margin-right:4px">ماركة</span>' : ''}</strong><br><span style="font-size:.75rem;color:var(--text-muted)">${products.filter(p => getProductCats(p).includes(c.name)).length} منتج</span></div>
          <button class="admin-btn admin-btn-danger admin-btn-sm" onclick="adminDeleteCategory(${idx})"><i class="fa-solid fa-trash"></i></button>
        </div>
