...
function adminLogin() {
  ...
  const wholesaleCode = (typeof adminSettings !== 'undefined' && adminSettings.wholesaleCode) ? adminSettings.wholesaleCode : 'ADMIN123';
  if (code.toLowerCase() === 'admin123' || code === wholesaleCode) {
...
function toggleAdminMktSubMenu(e, forceOpen = false) {
  ...
  const sub = document.getElementById('adminMktSubMenu') || document.getElementById('mktSubMenu');
  const chev = document.getElementById('adminMktChevron') || document.getElementById('mktChevron');
  if (!sub) return;
...
function initAdmin() {
  ...
  if (typeof switchAdminTab === 'function') {
    ...
    switchAdminTab(parts[0], parts[1] || '');
  } else {
    switchTab(hash);
  }
}
...
function switchTab(hash) {
  ...
  document.querySelectorAll('.tab-content, .admin-tab').forEach(function(t){
    t.classList.remove('active');
    t.style.display = 'none';
  });
  document.querySelectorAll('#sidebar nav button, .admin-sidebar button').forEach(function(b){ b.classList.remove('active'); });
  
  const el = document.getElementById('tab-' + tab) || document.getElementById('admin-' + tab);
  if (el) {
  ...
  if (tab === 'marketing' && typeof adminRenderMarketing === 'function') adminRenderMarketing(subTab || 'seo');
...
function exportProductsJSONAdmin() {
  var products = (typeof window.products !== 'undefined' && window.products.length) ? window.products : JSON.parse(localStorage.getItem('mycart_admin_products') || '[]');
    ...
    reader.onload = function(event) {
      try {
        ...
        if (Array.isArray(imported)) {
          var localProducts = JSON.parse(localStorage.getItem('mycart_admin_products') || '[]');
          var count = 0;
          ...
          localStorage.setItem('mycart_admin_products', JSON.stringify(localProducts));
          renderProducts();
...
function renderProducts() {
  ...
  if (!container) return;
  var products = JSON.parse(localStorage.getItem('mycart_admin_products') || '[]');
...
function deleteProductAdmin(idx) {
  var localProducts = JSON.parse(localStorage.getItem('mycart_admin_products') || '[]');
  if (!confirm('هل أنت تأكد من حذف المنتج "' + localProducts[idx].name + '"؟')) return;
...
function editProduct(idx) {
  if (typeof adminEditProduct === 'function') {
    adminEditProduct(idx);
  } else {
...
function adminEditProduct(idx) {
  ...
  else if (typeof switchAdminTab === 'function') switchAdminTab('addProduct');
  ...
  var oldPriceEl = document.getElementById('pOldPrice');
  var badgeEl = document.getElementById('pBadge');
  var featEl = document.getElementById('pFeatures');
  ...
  if (featEl && Array.isArray(p.features)) featEl.value = p.features.join('\n');
  if (editIdEl) editIdEl.value = idx;
  editingId = idx;
}
...
// Orders details handled by store.js adminShowOrderDetail
...
function switchAppSec(btn) {
  ...
  document.querySelectorAll('.app-panel').forEach(function(p){ p.classList.remove('active'); p.style.display = 'none'; });
  btn.classList.add('active');
  var target = document.getElementById('appsec-' + sec);
  if (target) { target.classList.add('active'); target.style.display = 'block'; }
}

function showNotifPanel() {
  if (typeof toggleAdminNotifDropdown === 'function') toggleAdminNotifDropdown();
  else if (typeof openNotifModal === 'function') openNotifModal();
}
...
function handleHashChangeAdmin() {
  ...
  var sub = parts[1] || '';
  if (tab === 'orderDetail') {
    if (typeof adminShowOrderDetail === 'function') adminShowOrderDetail(sub);
  } else if (tab === 'addProduct' && sub) {
    if (typeof adminEditProduct === 'function') {
      adminEditProduct(sub);
    } else if (typeof switchAdminTab === 'function') {
      switchAdminTab('addProduct');
    }
  } else if (typeof switchAdminTab === 'function') {
    switchAdminTab(tab, sub);
  }
}
