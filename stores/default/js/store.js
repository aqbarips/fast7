...
function initFilterRange() {
  ...
  if (maxEl) { minEl.min = minP; maxEl.max = maxP; maxEl.value = maxP; }
...
function adminUploadCatImage(inputId = 'acImage', previewId = 'acPreview', containerId = 'acPreviewContainer') {
  ...
  input.onchange = function(e) {
    ...
    if (file.size > 5 * 1024 * 1024) {
      if (typeof showToast === 'function') showToast('⚠️ الصورة كبيرة جداً', 'error');
      else alert('الصورة كبيرة جداً');
      return;
    }
    ...
    reader.onload = function(ev) {
      ...
      const container = containerId ? document.getElementById(containerId) : (preview ? preview.parentElement : null);

      if (preview) { preview.src = base64Data; preview.style.display = 'block'; }
      if (container) container.style.display = 'block';
      if (imgInput) imgInput.value = '⏳ جاري رفع الصورة...';

      var storeId = (typeof STORE_ID !== 'undefined') ? STORE_ID : 'default';
      fetch('/api/upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ data: base64Data, filename: file.name, store_id: storeId })
      })
      ...
      .then(function(res) {
        if (res && res.url) {
          ...
          if (typeof showToast === 'function') showToast('☁️ تم رفع الصورة أونلاين بنجاح', 'success');
        } else {
...
function getDeletedNotifIdsStore() {
  try { var r = localStorage.getItem('mycart_deleted_notifications'); return r ? JSON.parse(r) : []; } catch(e) { return []; }
}
...
function adminDeleteSingleNotif(e, id) {
  if (e) { e.preventDefault(); e.stopPropagation(); }
  try {
    var deleted = getDeletedNotifIdsStore();
    var sid = String(id);
    if (deleted.indexOf(sid) === -1) {
      deleted.push(sid);
      localStorage.setItem('mycart_deleted_notifications', JSON.stringify(deleted));
    }
  } catch(err) {}
  checkAdminNewOrders();
  closeAdminNotifDropdown();
  showAdminNotifPanel();
  if (typeof showToast === 'function') showToast('🗑️ تم إزالة الإشعار بنجاح', 'info');
}

function adminDeleteAllNotifs() {
  try {
    var deleted = getDeletedNotifIdsStore();
    var agencyNotifs = JSON.parse(localStorage.getItem('mycart_store_notifications') || localStorage.getItem('mycart_store_notifications_default') || '[]');
    agencyNotifs.forEach(function(n, i){
      var nid = String(n.id || ('agency_' + i));
      if (deleted.indexOf(nid) === -1) deleted.push(nid);
    });
    var orders = JSON.parse(localStorage.getItem('mycart_orders') || '[]');
    orders.forEach(function(o){
      var oid = String(o.id);
      if (deleted.indexOf(oid) === -1) deleted.push(oid);
    });
    localStorage.setItem('mycart_deleted_notifications', JSON.stringify(deleted));
  } catch(err) {}
  checkAdminNewOrders();
  closeAdminNotifDropdown();
  showAdminNotifPanel();
  if (typeof showToast === 'function') showToast('🗑️ تم مسح جميع الإشعارات', 'info');
}
...
function checkAdminNewOrders() {
  ...
  var deletedIds = getDeletedNotifIdsStore();

  var unreadAgency = agencyNotifs.filter(function(n, i){
    var nid = String(n.id || 'agency_'+i);
    return deletedIds.indexOf(nid) === -1 && readIds.indexOf(nid) === -1;
  }).length;

  var orders = JSON.parse(localStorage.getItem('mycart_orders') || '[]');
  var unreadOrders = orders.filter(function(o){
    var oid = String(o.id);
    return deletedIds.indexOf(oid) === -1 && readIds.indexOf(oid) === -1;
  }).length;
...
function showAdminNotifPanel() {
  ...
  if (agencyNotifs.length) {
    ...
    agencyNotifs.forEach(function(n, idx){
      var nid = String(n.id || ('agency_' + idx));
      if (deletedIds.indexOf(nid) !== -1) return;
      ...
      itemsHtml += '<div onclick="handleAdminNotifClick(\''+nid+'\', function(){ '+actionFnCode+'; })" style="display:flex;flex-direction:column;gap:6px;padding:10px 12px;border-radius:10px;margin-bottom:6px;background:'+cardBg+';border:1.5px solid '+cardBorder+';cursor:pointer;opacity:'+(isRead ? '0.75' : '1')+';position:relative">'
        + '<div style="display:flex;align-items:center;gap:8px">'
        ...
        + '<div style="flex:1;min-width:0"><div style="font-size:.78rem;font-weight:800;color:'+titleColor+';display:flex;align-items:center;justify-content:space-between"><span>'+(n.type==='payment'?'⚠️ ':'')+n.title+(isRead ? ' <span style="font-size:.58rem;color:#94a3b8;font-weight:400">(مقروء)</span>' : '')+'</span><button onclick="adminDeleteSingleNotif(event, \''+nid+'\')" title="حذف الإشعار" style="background:none;border:none;color:#cbd5e1;cursor:pointer;padding:2px;font-size:.75rem;transition:color .15s" onmouseover="this.style.color=\'#ef4444\'" onmouseout="this.style.color=\'#cbd5e1\'"><i class="fa-solid fa-trash-can"></i></button></div>'
  ...
  recent.forEach(function(o, i){
    var oid = String(o.id);
    if (deletedIds.indexOf(oid) !== -1) return;
    ...
    itemsHtml += '<div onclick="handleAdminNotifClick(\''+oid+'\', function(){ adminShowOrderDetail('+i+'); })" style="display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:8px;cursor:pointer;transition:background .15s;margin-bottom:'+(i<recent.length-1?'4px':'0')+';background:'+(isReadOrder ? '#f8fafc' : '#fef2f2')+';border:1px solid '+(isReadOrder ? '#e2e8f0' : '#fecaca')+';opacity:'+(isReadOrder ? '0.75' : '1')+'">'
      ...
      + '<button onclick="adminDeleteSingleNotif(event, \''+oid+'\')" title="حذف الإشعار" style="background:none;border:none;color:#cbd5e1;cursor:pointer;padding:2px 4px;font-size:.75rem;margin-right:2px;transition:color .15s" onmouseover="this.style.color=\'#ef4444\'" onmouseout="this.style.color=\'#cbd5e1\'"><i class="fa-solid fa-trash-can"></i></button>'
      + '</div>';
  });

  var headerControlHtml = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;gap:6px">'
    + '<div style="display:flex;align-items:center;gap:4px">'
    + '<button onclick="filterQuickAdminNotifs(\'all\')" style="background:'+(_quickAdminNotifFilter==='all'?'#8b5cf6':'#f1f5f9')+';color:'+(_quickAdminNotifFilter==='all'?'#fff':'#64748b')+';border:none;padding:3px 8px;border-radius:999px;font-size:.62rem;font-weight:800;cursor:pointer">الكل</button>'
    + '<button onclick="filterQuickAdminNotifs(\'unread\')" style="background:'+(_quickAdminNotifFilter==='unread'?'#8b5cf6':'#f1f5f9')+';color:'+(_quickAdminNotifFilter==='unread'?'#fff':'#64748b')+';border:none;padding:3px 8px;border-radius:999px;font-size:.62rem;font-weight:800;cursor:pointer">غير مقروء ⚡</button>'
    + '</div>'
    + '<div style="display:flex;align-items:center;gap:8px">'
    + '<button onclick="clearAllQuickAdminNotifs()" style="background:none;border:none;color:#8b5cf6;font-size:.62rem;font-weight:800;cursor:pointer;display:flex;align-items:center;gap:3px"><i class="fa-solid fa-check-double"></i> مقروء</button>'
    + '<button onclick="adminDeleteAllNotifs()" style="background:none;border:none;color:#ef4444;font-size:.62rem;font-weight:800;cursor:pointer;display:flex;align-items:center;gap:3px"><i class="fa-solid fa-trash-can"></i> مسح الكل</button>'
    + '</div>'
    + '</div>';
