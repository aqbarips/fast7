...
function adminEditCategory(idx) {
  ...
  modal.innerHTML = `
    <div style="background:var(--card, #fff);border-radius:20px;padding:24px;max-width:460px;width:100%;box-shadow:0 24px 48px rgba(0,0,0,.2);border:1px solid var(--border,#e2e8f0);position:relative">
      <button onclick="document.getElementById('adminCatEditModal').style.display='none'" style="position:absolute;top:14px;left:14px;background:none;border:none;font-size:1.3rem;cursor:pointer;color:#94a3b8">×</button>
      <h3 style="margin-bottom:16px;font-weight:900;font-size:1.15rem;color:var(--text,#0f172a);display:flex;align-items:center;gap:8px">
      ...
      <div class="admin-form-group" style="margin-bottom:14px">
        <label style="display:block;font-size:.82rem;font-weight:800;margin-bottom:6px;color:var(--text,#0f172a)">اسم التصنيف *</label>
        <input type="text" id="editCatName" value="${cat.name || ''}" style="width:100%;padding:10px 14px;border:1.5px solid var(--border,#cbd5e1);border-radius:10px;font-family:inherit;font-size:.88rem;box-sizing:border-box">
      </div>
      
      <div class="admin-form-group" style="margin-bottom:14px">
        <label style="display:block;font-size:.82rem;font-weight:800;margin-bottom:6px;color:var(--text,#0f172a)">صورة التصنيف</label>
        <div style="display:flex;gap:8px;align-items:center">
          <input type="text" id="editCatImage" value="${cat.image || ''}" placeholder="ضع رابط الصورة أو ارفع صورة..." style="flex:1;padding:10px 14px;border:1.5px solid var(--border,#cbd5e1);border-radius:10px;font-family:inherit;font-size:.85rem;box-sizing:border-box">
          <button class="admin-btn admin-btn-secondary" onclick="adminUploadCatImage('editCatImage', 'editCatPreview', 'editCatPreviewContainer')" style="padding:10px 16px;border-radius:10px;background:#f1f5f9;border:1px solid #cbd5e1;cursor:pointer;font-weight:700;font-family:inherit;display:flex;align-items:center;gap:6px">
            <i class="fa-solid fa-cloud-arrow-up"></i> رفع
          </button>
        </div>
        <div id="editCatPreviewContainer" style="margin-top:10px;display:${cat.image ? 'block' : 'none'};background:#f8fafc;border:1.5px dashed #cbd5e1;border-radius:14px;padding:12px;text-align:center">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
            <span style="font-size:.75rem;font-weight:700;color:#64748b">🖼️ معاينة الصورة:</span>
            <button onclick="adminRemoveCatImage('editCatImage', 'editCatPreviewContainer')" style="background:#fef2f2;color:#ef4444;border:1px solid #fecaca;border-radius:6px;padding:2px 8px;font-size:.72rem;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:4px">
              <i class="fa-solid fa-trash-can"></i> إزالة الصورة
            </button>
          </div>
          <img id="editCatPreview" src="${cat.image || ''}" style="max-width:120px;max-height:120px;border-radius:12px;object-fit:cover;border:2px solid #fff;box-shadow:0 4px 12px rgba(0,0,0,.1);margin:0 auto;display:block">
        </div>
      </div>

      <div class="admin-form-group" style="margin-bottom:20px">
        <div style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:14px;padding:12px 14px;display:flex;align-items:center;justify-content:space-between;cursor:pointer;transition:all .2s" onclick="var cb=document.getElementById('editCatIsBrand');cb.checked=!cb.checked">
          <div style="display:flex;align-items:center;gap:10px">
            <div style="width:36px;height:36px;border-radius:10px;background:#eff6ff;color:#2563eb;display:flex;align-items:center;justify-content:center;font-size:1.1rem">
              <i class="fa-solid fa-award"></i>
            </div>
          ...
          <input type="checkbox" id="editCatIsBrand" ${cat.isBrand ? 'checked' : ''} style="width:20px;height:20px;accent-color:var(--accent,#ef4444);cursor:pointer" onclick="event.stopPropagation()">
        </div>
      </div>

      <div style="display:flex;gap:10px;justify-content:flex-end">
        <button class="admin-btn admin-btn-secondary" onclick="document.getElementById('adminCatEditModal').style.display='none'" style="padding:10px 18px;border-radius:10px;border:1px solid #cbd5e1;background:#f1f5f9;cursor:pointer;font-weight:700;font-family:inherit">إلغاء</button>
        <button class="admin-btn admin-btn-primary" onclick="adminSaveEditCategory(${idx})" style="padding:10px 20px;border-radius:10px;border:none;background:var(--accent,#ef4444);color:#fff;cursor:pointer;font-weight:700;font-family:inherit">حفظ التغييرات</button>
      </div>
    </div>
  `;
  ...
  if (imgInput) {
    imgInput.oninput = function() {
      const container = document.getElementById('editCatPreviewContainer');
...
function adminAddCategory() {
  ...
  const acPrev = document.getElementById('acPreview');
  if (acPrev) acPrev.style.display = 'none';
  const addModal = document.getElementById('adminCatAddModal');
